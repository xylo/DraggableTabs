 ...
 */
public class DraggableTabDemoWithSubTabs extends Application {
	...
	public void start(Stage stage) {
		stage.setTitle("DraggableTabDemo2");
		...
		for (TabPane tabPane : Arrays.asList(tabPane1, tabPane2)) {
			for (int i = 1; i <= TABS_PER_PANE; i++) {
				...
				final DraggableTabLayoutExtender sub = new DraggableTabLayoutExtender(
						new DraggableTabPane(new DraggableTab("Sub Tab " + i))
				);

				tab.setContent(new SplitPane(sub) {{
